...
- Keep Hosted Renovate compatibility validation pinned through `.github/renovate-version`; do not reintroduce a self-hosted workflow only to pin the Renovate version.
- Keep `mise.lock` generation independent of where Renovate runs: PR workflows may generate candidates with read-only permissions, but only the default-branch `workflow_run` reconciler may write the validated lockfile through a current-repository, Contents-write token minted from the maintainer App. Never expose that App secret/token to `pull_request`, execute PR code in the privileged job, force-update a PR ref, or let Renovate run `mise` through `allowedUnsafeExecutions`/`postUpgradeTasks`.
