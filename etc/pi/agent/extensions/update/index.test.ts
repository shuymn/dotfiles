...
function defaultExec(call: ExecCall): ExecResult {
  ...
  if (
    call.command === "bun" &&
    args.startsWith("install -g @earendil-works/pi-coding-agent@")
  )
...
describe("update extension", () => {
  ...
  test("reports already-current version and skips audit/install", async () => {
    ...
    setFetch((url, init) => {
      expect(url).toBe(
        "https://npm.flatt.tech/@earendil-works%2Fpi-coding-agent/latest",
      );
  ...
  test("audits pinned package, installs latest version, verifies version, and shuts down", async () => {
    ...
    expect(ctx.notifications.map((item) => item.level)).toEqual([
      "info",
      "info",
      "info",
      "info",
