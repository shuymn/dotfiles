...
function defaultExec(call: ExecCall): ExecResult {
  return {
    code: 0,
    stdout: JSON.stringify({ ok: true, command: call.args[0], args: call.args }),
    stderr: "",
  };
}
...
describe("tavily extension", () => {
  test("registers all Tavily tools with schemas and guidance", async () => {
    ...
    expect(tools.get("tavily_search")!).toMatchObject({
      label: "Tavily Search",
      parameters: { type: "object", properties: { query: { type: "string" }, depth: { enum: ["ultra-fast", "fast", "basic", "advanced"], optional: true } } },
    });
    expect(tools.get("tavily_search")!.promptGuidelines!.join("\n")).toContain("Keep tavily_search queries under 400 characters");
    expect(tools.get("tavily_auth_status")!.parameters).toEqual({ type: "object", properties: {} });
  });
  ...
  test("extract validates chunksPerSource dependency and computes timeout", async () => {
    ...
    await expect(
      tools.get("tavily_extract")!.execute("call", { urls: ["https://example.com"], chunksPerSource: 2 }, undefined, undefined),
    ...
    expect(pi.execCalls.at(-1)).toEqual({
      command: "tvly",
      args: ["extract", "https://example.com/a", "https://example.com/b", "--json", "--query", "pricing", "--chunks-per-source", "2", "--extract-depth", "advanced", "--format", "text", "--include-images", "--timeout", "5"],
  ...
  test("map and crawl build site args with default and custom timeouts", async () => {
    ...
    expect(pi.execCalls.at(-1)).toEqual({
      command: "tvly",
      args: ["map", "https://docs.example.com", "--json", "--max-depth", "2", "--max-breadth", "5", "--limit", "20", "--instructions", "docs only", "--select-paths", "/docs", "--exclude-paths", "/blog", "--allow-external"],
      options: { signal: undefined, timeout: 160_000 },
    });

    await expect(
      tools.get("tavily_crawl")!.execute("call", { url: "https://docs.example.com", chunksPerSource: 2 }, undefined, undefined),
    ...
    expect(pi.execCalls.at(-1)).toEqual({
      command: "tvly",
      args: ["crawl", "https://docs.example.com", "--json", "--max-depth", "3", "--max-breadth", "4", "--limit", "30", "--instructions", "api docs", "--chunks-per-source", "2", "--extract-depth", "basic", "--format", "markdown", "--select-paths", "/api,/guide", "--exclude-paths", "/old", "--select-domains", "docs.example.com", "--exclude-domains", "cdn.example.com", "--include-images", "--timeout", "20"],
  ...
  test("auth status runs tvly auth with auth timeout", async () => {
    ...
    expect(pi.execCalls).toEqual([{ command: "tvly", args: ["auth", "--json"], options: { signal: undefined, timeout: 20_000 } }]);
  });

  test("non-JSON stdout is returned as text and stderr is included on command failure", async () => {
    const { tools } = await loadTools(() => ({ code: 2, stdout: "plain output", stderr: "bad auth" }));

    await expect(tools.get("tavily_auth_status")!.execute("call", {}, undefined, undefined)).rejects.toThrow(
  ...
  test("truncates oversized rendered output", async () => {
    ...
    const { tools } = await loadTools(() => ({ code: 0, stdout: longText, stderr: "" }));
