...
mock.module("@earendil-works/pi-tui", () => ({
  ...
  SelectList: class implements SelectInstance {
    ...
    onSelect?: (item: { value: string; label: string; description?: string }) => void;
    onCancel?: () => void;
    constructor(public items: Array<{ value: string; label: string; description?: string }>) {
...
function defaultExec(call: ExecCall): ExecResult {
  ...
  if (joined === "symbolic-ref --quiet --short refs/remotes/origin/HEAD") return { code: 0, stdout: "origin/main\n", stderr: "" };
  if (joined === "for-each-ref --format=%(refname)%09%(refname:short) refs/heads refs/remotes") {
    return { code: 0, stdout: "refs/heads/feature\tfeature\nrefs/heads/main\tmain\nrefs/remotes/origin/main\torigin/main\nrefs/remotes/origin/HEAD\torigin/HEAD\n", stderr: "" };
  }
  if (joined === "config --get user.email") return { code: 0, stdout: "dev@example.com\n", stderr: "" };
  if (joined === "status --short") return { code: 0, stdout: " M src/app.ts\n", stderr: "" };
  if (joined === "branch --show-current") return { code: 0, stdout: "feature\n", stderr: "" };
  if (joined === "log --author=dev@example\\.com --format=%s -10") return { code: 0, stdout: "feat: self commit\n", stderr: "" };
...
describe("commit extension", () => {
  test("registers --commit flag and lifecycle guards", async () => {
    ...
    expect([...pi.events.keys()].sort()).toEqual(["agent_end", "session_start", "tool_call"]);
  });
  ...
  test("shuts down with Japanese warning when the workflow cannot start", async () => {
    ...
    expect(noUiCtx.notifications).toEqual([{ message: "--commit には対話式 UI が必要です", level: "warning" }]);
  ...
  test("cancels cleanly when option collection is cancelled", async () => {
    ...
    expect(ctx.notifications).toEqual([{ message: "--commit をキャンセルしました", level: "info" }]);
  ...
  test("starts commit workflow with selected options and a git snapshot", async () => {
    ...
    expect(prompt).toContain("### Recent Self Commits (primary for auto language)\nfeat: self commit");
  ...
  test("branch creation flow discovers branches and includes base branch flag", async () => {
    ...
    expect(pi.sentUserMessages[0]).toContain("User invoked --commit with interactive options: --english --branch --base=main");
  ...
  test("blocks destructive git commands and push only while commit workflow is active", async () => {
    ...
    const inactiveResult = await pi.events.get("tool_call")![0]({ toolName: "bash", input: { command: "git reset --hard" } });
    ...
    for (const command of destructiveCommands) {
      const result = await pi.events.get("tool_call")![0]({ toolName: "bash", input: { command } });
      expect(result).toEqual({
        block: true,
        reason: "/commit extension によりブロックしました: コミット準備中の破壊的な git cleanup/reset コマンドは禁止されています。代わりにユーザーへ明示的な指示を確認してください。",
      });
    }

    await expect(pi.events.get("tool_call")![0]({ toolName: "bash", input: { command: "git status --short" } })).resolves.toBeUndefined();
    await expect(pi.events.get("tool_call")![0]({ toolName: "read", input: { command: "git reset --hard" } })).resolves.toBeUndefined();
    await expect(pi.events.get("tool_call")![0]({ toolName: "bash", input: { command: "git push origin HEAD" } })).resolves.toEqual({
      block: true,
      reason: "/commit extension によりブロックしました: /commit はローカルコミットのみを作成します。push しないでください。",
    });
  });
